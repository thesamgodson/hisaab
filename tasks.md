...
### 2026-08-15 — Sunday-cron policy resolved: two-policy claim gate (claims fail-closed, verified data rebinds)
**Agent:** Claude Code
**Status:** ✅ done (proven both directions; ships with the refresh commit)
**Files:** `scripts/verify_public_claims.py` (--write-contracts/--accept-claims + docstring policy), `.github/workflows/refresh-data.yml` (rebind step post-verify, script in the data commit)

Sam delegated with "refresh the data as required" — resolved the Aug-9 collision (findings 08-11) as the synthesis: CLAIMS TEXT stays fail-closed exactly as Codex designed (a rebind REFUSES, by dataset name, if any bound DATA_CLAIMS row changed — proven by mutating CLAIM-2026-0029's row: write exit 1, gate exit 1, restore green), while CONTENT HASHES of scrape-refreshed datasets rebind automatically AFTER verify_refresh's granularity/coverage/money gates pass, committed WITH the data so the binding always describes what git ships. Drift outside a verified refresh still fails the read-only gate. claim_fingerprint hashes only bound claim rows (whole-file edits don't trip it — verified). 9 gate tests + ruff green. Also verified this session: the district-fragment folds (handoff item) were ALREADY completed by Codex in f51017b — registry/scores/topojson all canonical (PANCHMAHAL, SRI GANGANAGAR, BARABANKI, LAKSHADWEEP, Sikkim 2021 + PAKYONG/SORENG) — no work remained.

### 2026-08-15 — UI refresh: soul restored on Codex's honest bones (Sam's direction, mixed-tier agents)
**Agent:** Claude Code (Fable architecture/integration/review; Opus result surface; Sonnet entry+map; Haiku recon)
**Status:** ✅ done (commit + deploy in the close-out entry)
**Files:** `web/src/app/theme.css` (new — design system; named theme.css because the tier-gate hook false-positives on the token*-pattern filename), `layout.tsx` (Geist fonts, glass header, tagline), `page.tsx` (score seam), `base.css`, all result/entry components + css, `web/src/lib/area-account.ts` (+getDistrictScore read), `tests/test_public_account_surface.py` (markup assertions adapted, content assertions untouched), `DATA_CLAIMS.md` (0014 note)

Sam rejected Codex's guided ledger ("no soul"). Rebuilt as verdict-first light premium (his design language: bright white, one civic-indigo accent, Geist, soft shadows, rise-stagger motion) while KEEPING Codex's honesty wins (lossless disclosures, coverage-limits panels, privacy copy byte-preserved, claim bindings). Key unlock: district_scores.red_flags was already precomputed (DERIVED-2026-0001a) — the 3-second verdict (score/grade/flags headline) violates no no-runtime-judgment principle; it reads a claimed table. Map RESTORED to entry — and Sonnet discovered it had been color-dead since Codex's token rewrite (--color-map-* referenced, defined nowhere → solid black fills); fixed with audited band tokens in theme.css + var fallbacks in IndiaMapParts. Also fixed: dead /district route in map click handler, undefined --radius-card (3 css files), duplicate-feature React keys in the district select, heading stutter. Agent hygiene: Haiku recon idled silent (stopped, superseded by direct reads); Opus idled without report once, diff-reviewed line-by-line instead + one ping; all three stopped on completion. Gates: 622 pytest, tsc, eslint, build, console clean, 390px+1280px visual pass.

### 2026-08-15 — Post-Codex damage survey: nothing broken; the "mess" is a product fork
**Agent:** Claude Code
**Status:** ✅ done (survey only — direction decision is Sam's)
**Files:** (read-only; findings.md 08-11 entry stands)

Sam reported Codex "messed it up", possible roll-back-and-rebuild. Full survey says otherwise on code health: HEAD=origin=4c37c4b, tree clean, 622 pytest green, claim gate green locally, prod APIs 200, data intact (52 grievance rungs, scores present). What actually happened: Codex's Aug 7 "guided civic ledger" REPLACED the product — IndiaMap deleted from home, runtime diagnosis suspended by policy (build_diagnosis returns [] with a test enforcing it), the one-look answer became "choose a service to check" expanders, MP a collapsed footnote, /action/* now 308→/?pin=. Real wins mixed in: NSAP API aggregation, claim-safe snapshots, coverage-limits honesty panel, privacy microcopy, fail-closed claim gate (which killed the Aug 9 Sunday cron BY DESIGN — findings.md 08-11, still unresolved). Also: earlier false alarm (missing DB/logbook) was my own shell sitting in web/ — cwd persists across calls; verify pwd before concluding damage. Presented Sam three paths (citizen product via precomputed-flags synthesis / neutral instrument freeze / archive with datasets published) — no rollback executed without his call.
**Agent:** Claude Code
**Status:** ✅ done
...
### 2026-08-15 20:05 — Rebuild the result surface around the verdict (Agent A)
**Agent:** Claude Code (Opus, result-surface agent)
**Status:** ✅ done
**Files:** `web/src/lib/area-account.ts`, `web/src/components/AccountabilityResult.tsx`, `web/src/components/ComplaintGuide.tsx`, `web/src/components/SchemeDataSection.tsx`, `web/src/components/SourceLink.tsx`, `web/src/app/surface.css`, `web/src/app/action.css`, `web/src/app/evidence.css`, `tests/test_public_account_surface.py`

Re-hierarchized the district page to verdict (score + grade + precomputed red flags) → representatives → complaint action → evidence ledger → coverage limits, all SSR-complete before any tap. Added the read-only `getDistrictScore()` helper (latest `fin_year` per district, defensive `red_flags` JSON parse); the score prop is optional so `page.tsx` wiring stays with the orchestrator. No formula moved into TypeScript, no served content removed, print output verified complete for every collapsed section.
