...
| `action_brief/` | PIN/district → sourced complaint routes + area context (Python; TS twin in `web/src/lib/action-brief.ts`). Complaint kits come from the complete CLAIM-backed entitlement + grievance registries; district data never creates, hides, orders, or preselects a citizen's issue. Runtime severity formulas are suspended until one registered load-time contract can serve both twins |
| `constituency/` | PIN/constituency/MP/MLA ingest + district lineage & UT-seat seeds + `pc_name_registry.py` (canonical 2024 PC names, state-scoped, applied at every label-writing boundary; TS twin generated by `scripts/gen_pc_name_registry.py`) |
...
| `scripts/` | `sync_turso.py` (publish→prod), `gen_district_aliases.py`, `gen_pc_name_registry.py`, `build_pin_geo.py` (GeoNames PIN centroids), `data_audit.py`, `build_geodata.py` |
...
- Financial data: district-level MGNREGA + PMGSY (real), NSAP (imputed), NRLM RF+CIF (real, cumulative); state-level PM POSHAN (2016-25), NSAP (2019-24), PMAY-G (2019-26, un-gated B.3 report — captcha removed 2026-08-04), JJM (2019-25), NFSA (MT). PM Kisan money is frozen at 8 states' FY2024-25 rows
