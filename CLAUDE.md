...
| `action_brief/` | PIN → diagnosis → contacts → actions engine (Python; TS twin in `web/src/lib/action-brief.ts`). Complaint kits (WHY `scheme_entitlements` / WHO+HOW `grievance_channels`, both curated + CLAIM-backed) render per scheme PRESENT in the district — never gated on a flagged shortfall |
